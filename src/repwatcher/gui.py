...
def post_game(game: Game) -> None:
    ...
    ttk.Combobox(mainframe, values=[game.player1, game.player2], textvariable=winnervar, state="readonly").grid(column=2, row=0, columnspan=2)
    ...
    ttk.Label(mainframe, text=f"{float(game.duration) // 60:.0f}:{game.duration % 60:02.0f}").grid(column=3, row=4)
    ...
    ttk.Combobox(mainframe, values=p1buildorders, textvariable=p1bo).grid(column=3, row=1)
    
    ttk.Label(mainframe, text="Build Order").grid(column=2, row=2)
    p2bo = StringVar(value=game.buildorder2)
    ttk.Combobox(mainframe, values=p2buildorders, textvariable=p2bo).grid(column=3, row=2)
    ...
    def save_game():
        ...
        if bo := p1bo.get():
            if bo not in p1buildorders:
                BuildOrder.create(buildorder=bo, race=game.player1race, vs=game.player2race)
            game.buildorder1 = bo
        if bo := p2bo.get():
            if bo not in p2buildorders:
                BuildOrder.create(buildorder=bo, race=game.player2race, vs=game.player1race)
            game.buildorder2 = bo                      
        game.save()
        app.destroy()
    ...
    open_url = ttk.Button(buttonframe, text="Open URL", command=lambda: webbrowser.open(game.url))
    save = ttk.Button(buttonframe, text="Save", command=save_game)
    open_replay = ttk.Button(buttonframe, text="Open Replay (SB)", command=open_replay_cmd)
