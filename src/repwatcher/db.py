...
class BuildOrder(BaseModel):
    ...
    def get_buildorders_from_matchup(Game) -> tuple[list[str], list[str]]:
        p1 = [x.buildorder for x in BuildOrder.select().where(BuildOrder.race == Game.player1race, BuildOrder.vs == Game.player2race)] # type: ignore
        p2 = [x.buildorder for x in BuildOrder.select().where(BuildOrder.race == Game.player2race, BuildOrder.vs == Game.player1race)] # type: ignore
        return p1, p2
...
def create_default_build_orders():
    ...
    for bo in default_build_orders:
        BuildOrder.insert([bo.buildorder, bo.race, bo.vs]).on_conflict_ignore().execute()